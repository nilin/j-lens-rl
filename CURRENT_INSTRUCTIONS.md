...
Last reconciled: 2026-07-14 22:50 UTC. This file is the current operating
...
The current and final candidate is the V12 celebration-tail infrastructure
replacement:
...
seeds 224--227. Training is six fixed updates, LR `3e-6`, DAPO, LoRA r8/alpha
...
- Run at most five Modal GPUs before 23:00 UTC, then at most ten. The registered
  V12 launcher uses four treatment GPUs and only conditionally four controls.
- Spend at least half of experiment/GPU time on RL. Never hold ready RL behind
  word search for over an hour. Do not reopen candidate selection after V12.
...
recipe choice from its outcomes. V11 celebration is also terminal: its complete
four-seed means rose `.3825 -> .3925 -> .396875` through steps `0/4/5`, but a
post-training verifier/log-shape mismatch propagated early and interrupted two
step-6 evaluations. It has no evaluable gate, controls, or final. V12 preserves
its exact science but uses wholly fresh execution identities and excludes all
V11 rows from inference; describe any V12 `p=.125` only as nominal adaptive-
program evidence, never as an independent untouched replication.
