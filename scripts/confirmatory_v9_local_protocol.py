#!/usr/bin/env python3
"""Prepare and run a fresh whole RTX-4090/offline-W&B V9 replication.

V9-local preserves V8's emotional profanity J-lens science and local backend,
while changing the prospectively registered seeds, isolated state/identities,
and the terminal verifier that caused V8 to fail closed.  The 400-row curve is
already exposed and is only an operational consistency gate.  The 900-row final
remains sealed until all registered gates pass and is the sole inferential
allocation.
"""
...
STATE_DIR = REPO / ".confirmatory" / "v9_local"
...
REGISTRATION_PROTOCOL = "j-lens-rl-confirmatory-v9-local-profanity-registration-v1"
...
LOCAL_GPU_LOCK = Path("/tmp/jlens-rl-v9-local-rtx4090.lock")
LOCAL_RUNTIME_ID = "j-lens-rl-confirmatory-v9-local-rtx4090-offline-v1"
...
V8_CLOSEOUT_PROTOCOL = "j-lens-rl-confirmatory-v8-local-terminal-closeout-v1"
V8_TERMINAL_STAGE = "infrastructure_failed_attempt_closed"
V8_CLOSEOUT_SHA256 = "d8c48d67b7ff037aeac55774bcc1fedf4b2a574283e7cf12151cf3c95b57e638"
V8_CLAIM_ID = "ff4144f0f3d14a49a669968c5c5a7a85"
V8_REGISTRATION_SHA256 = (
    "10c1969a8f6e5f8c5caede68019b627a19ae8121daaea31ec91715525ba98090"
)
V8_RECIPE_SHA256 = "78103d223d7bd5eddf1485f4ce508d57bb83b5e1b0866c5f251f36072e415241"
V8_SOURCE_COMMIT = "af83249ffef8e62d83f1f4e2e975670506941532"
V8_FAILED_LABEL = "jlens_seed200"
V8_EVIDENCE_COUNT = 20
...
V8_CLOSEOUT_SOURCE = REPO / "protocol_archive" / "v8_local_terminal_closeout.json"
V8_TERMINAL_EVIDENCE_SOURCE = (
    REPO / "protocol_archive" / "v8_local_terminal_evidence"
)
RECIPE_SOURCE = REPO / "protocol_archive" / "v9_local_profanity_recipe_lock.json"
REGISTRATION_DRAFT_SOURCE = (
    REPO / "protocol_archive" / "v9_local_profanity_registration_draft.json"
)
AMENDMENT_SOURCE = REPO / "protocol_archive" / "v9_local_prelaunch_amendment.json"
AMENDMENT_TEMPLATE_SOURCE = (
    REPO / "protocol_archive" / "v9_local_prelaunch_amendment_TEMPLATE.json"
)
RUNTIME_ALLOWLIST_SOURCE = (
    REPO / "scripts" / "v9_local_runtime_source_allowlist.json"
)

TRAIN_CODE_PATHS = (
    "scripts/confirmatory_v9_local_protocol.py",
    "scripts/confirmatory_v9_local_runner.py",
    "scripts/v9_local_train.py",
    "scripts/v9_local_runtime_source_allowlist.json",
    "run_confirmatory_v9_local.sh",
...
def _require_clean_pushed_main() -> dict[str, str]:
    ...
    if status:
        raise ProtocolError(f"V9-local preparation requires a clean tree:\n{status}")
    head = _git("rev-parse", "HEAD")
    if _git("rev-parse", "origin/main") != head:
        raise ProtocolError("V9-local preparation requires HEAD pushed to origin/main")
    return {
        "git_commit": head,
...
def _tree_identity(path: Path) -> dict[str, Any]:
    ...
    # This must exactly mirror src/jlens_rl/train.py::_tree_identity.  V8 was
    # closed after its first terminal run because its independent validator
    # accidentally omitted this absolute path field.
    return {
...
def _source_inputs() -> dict[str, str]:
    expected = {
        ...
        str(V8_CLOSEOUT_SOURCE.relative_to(REPO)): V8_CLOSEOUT_SHA256,
    }
    actual = {name: sha256_file(REPO / name) for name in expected}
    if actual != expected:
        raise ProtocolError(f"V9-local fixed input changed: {actual}")
...
def _validate_recipe(recipe: Any) -> dict[str, Any]:
    if (
        ...
        raise ProtocolError("V9-local recipe lock changed")
    ...
    if treatment != [
        ...
        raise ProtocolError("V9-local treatment/control transform changed")
    return recipe
...
def _validate_closeout(
    ...
    if (
        ...
        != "j-lens-rl-confirmatory-v9-local-prelaunch-amendment"
        ...
        raise ProtocolError("V9-local prelaunch amendment is malformed or disabled")
    ...
    if (
        ...
        raise ProtocolError("V9-local amendment lacks the authoritative V7 closeout")
    ...
    if (
        ...
        raise ProtocolError("V9-local amendment does not pin the exact lease retirement")
...
def _validate_v8_closeout(
    amendment: Any,
    copied_closeout: Path | None = None,
    copied_evidence_dir: Path | None = None,
) -> Path:
    """Validate the immutable V8 failure boundary and every archived evidence file."""
    if (
        not isinstance(amendment, dict)
        or amendment.get("v8_terminal_closeout_path")
        != "protocol_archive/v8_local_terminal_closeout.json"
        or amendment.get("v8_terminal_closeout_sha256") != V8_CLOSEOUT_SHA256
        or amendment.get("required_v8_closeout_protocol") != V8_CLOSEOUT_PROTOCOL
        or amendment.get("required_v8_terminal_stage") != V8_TERMINAL_STAGE
        or amendment.get("v8_claim_id") != V8_CLAIM_ID
        or amendment.get("v8_registration_sha256") != V8_REGISTRATION_SHA256
        or amendment.get("v8_recipe_lock_sha256") != V8_RECIPE_SHA256
        or amendment.get("v8_source_git_commit") != V8_SOURCE_COMMIT
        or amendment.get("v8_failed_label") != V8_FAILED_LABEL
        or amendment.get("require_v8_final_outcomes_unopened") is not True
        or amendment.get("require_v8_attempt_permanently_closed") is not True
    ):
        raise ProtocolError("V9-local amendment lacks the exact V8 closeout boundary")

    path = copied_closeout or V8_CLOSEOUT_SOURCE
    if not path.is_file() or sha256_file(path) != V8_CLOSEOUT_SHA256:
        raise ProtocolError("pinned V8 closeout is missing or changed")
    closeout = read_json(path)
    disposition = closeout.get("disposition", {})
    failure = closeout.get("failure", {})
    root_cause = failure.get("root_cause", {})
    boundary = closeout.get("final_boundary", {})
    identity = closeout.get("operational_identity", {})
    terminal = closeout.get("seed200_terminal_evidence", {})
    if (
        closeout.get("protocol") != V8_CLOSEOUT_PROTOCOL
        or disposition.get("attempt") != V8_TERMINAL_STAGE
        or "permanently closed" not in str(disposition.get("conclusion", ""))
        or failure.get("error") != "jlens_seed200 terminal result changed"
        or failure.get("error_type") != "ProtocolError"
        or failure.get("dispatch_completion_present") is not False
        or failure.get("gpu_processes_after_failure") != []
        or root_cause.get("trainer_helper")
        != "src/jlens_rl/train.py:_tree_identity writes path, sha256, and files"
        or root_cause.get("v8_validator_helper")
        != (
            "scripts/confirmatory_v8_local_protocol.py:_tree_identity expected "
            "only sha256 and files"
        )
        or set(root_cause.get("exact_actual_only_fields", {}))
        != {
            "$.terminal_checkpoint.path",
            "$.final_adapter_and_tokenizer.path",
        }
        or root_cause.get("all_other_terminal_result_fields_equal") is not True
        or boundary.get("curve_gate_evaluated") is not False
        or boundary.get("controls_started") is not False
        or boundary.get("final_unlocked") is not False
        or boundary.get("final_collection_present") is not False
        or boundary.get("final_evaluation_present") is not False
        or boundary.get("analysis_present") is not False
        or boundary.get("acceptance_present") is not False
        or boundary.get("sealed_900_item_final_unopened") is not True
        or identity.get("protocol") != "j-lens-rl-confirmatory-v8-local-profanity-u5"
        or identity.get("claim_id") != V8_CLAIM_ID
        or identity.get("registration_sha256") != V8_REGISTRATION_SHA256
        or identity.get("recipe_lock_sha256") != V8_RECIPE_SHA256
        or identity.get("source_git_commit") != V8_SOURCE_COMMIT
        or identity.get("max_parallel_gpu_workers") != 1
        or terminal.get("seed") != 200
        or terminal.get("updates") != 20
        or terminal.get("curve_steps") != [0, 4, 10, 20]
        or terminal.get("registered_eight_seed_gate_evaluated") is not False
        or terminal.get("offline_receipt_validator_passed") is not True
        or terminal.get("terminal_result_equal_after_adding_expected_path_fields")
        is not True
    ):
        raise ProtocolError("V8 closeout changed its failure or sealed-final boundary")

    evidence = closeout.get("durable_source_evidence")
    if (
        not isinstance(evidence, dict)
        or len(evidence) != V8_EVIDENCE_COUNT
        or any(
        ...
        raise ProtocolError("V8 closeout lacks its exact evidence inventory")
    evidence_root = copied_evidence_dir or V8_TERMINAL_EVIDENCE_SOURCE
    for relative, digest in evidence.items():
        ...
        if not evidence_path.is_file() or sha256_file(evidence_path) != digest:
            raise ProtocolError(f"V8 closeout evidence changed: {relative}")
    return path


def _amendment() -> tuple[dict[str, Any], Path | None, Path | None]:
    if AMENDMENT_SOURCE.is_file():
        value = read_json(AMENDMENT_SOURCE)
        return value, _validate_closeout(value), _validate_v8_closeout(value)
    value = read_json(AMENDMENT_TEMPLATE_SOURCE)
    if value.get("launch_enabled") is not False:
        raise ProtocolError("unbound V9-local amendment template enabled launch")
...
def _base_training_config(recipe: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "lens_path": ".confirmatory/v9_local/frozen_artifacts/lens.pt",
        "lens_sha256": LENS_SHA256,
        "expected_lens_sha256": LENS_SHA256,
        "calibration_path": ".confirmatory/v9_local/frozen_artifacts/calibration.json",
        ...
        "validation_indices_path": ".confirmatory/v9_local/manifests/curve_indices.json",
        "reserved_train_indices_path": ".confirmatory/v9_local/manifests/train_exclusions.json",
            ...
            config.update(
                {
                    ...
                    "metric_schema_path": (
                        ".confirmatory/v9_local/reproducibility/metric_schema.json"
                    ),
    ...
    sealed.update(
        {
            ...
            "evaluation_indices_path": (
                ".confirmatory/v9_local/manifests/sealed_final_indices.json"
            ),
            "evaluation_seed": 0,
            "min_new_tokens": 0,
            "output_dir": ".confirmatory/v9_local/evaluation_config_unused",
...
def _config_path(condition: str, seed: int) -> Path:
    if condition not in CONDITIONS or seed not in SEEDS:
        raise ProtocolError(f"unregistered V9-local run: {condition}/{seed}")
...
def _launch_plan(configs: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    for label in FINAL_LABELS:
        if label == "base":
            experiment = f".confirmatory/v9_local/configs/jlens_seed{SEEDS[0]}.json"
            adapter: list[str] = []
        else:
            experiment = f".confirmatory/v9_local/configs/{label}.json"
            adapter = ["--adapter", f".confirmatory/v9_local/runs/{label}/final"]
        evaluations[label] = [
            str(PYTHON_EXECUTABLE),
            "-m",
            "jlens_rl.eval",
            "--config",
            ".confirmatory/v9_local/configs/sealed_eval.json",
            "--experiment-config",
            experiment,
            "--indices-manifest",
            ".confirmatory/v9_local/manifests/sealed_final_indices.json",
            "--output-jsonl",
            f".confirmatory/v9_local/evals/{label}.jsonl",
            "--run-label",
            label,
            "--batch-size",
            "64",
    ...
    return {
        ...
        "analysis_command": [
            ...
            "--base-jsonl",
            ".confirmatory/v9_local/evals/base.jsonl",
            *[
                value
                for seed in SEEDS
                for value in (
                    "--adapter-jsonl",
                    f".confirmatory/v9_local/evals/jlens_seed{seed}.jsonl",
                )
            ],
            *[
                value
                ...
                for value in (
                    "--control-jsonl",
                    f".confirmatory/v9_local/evals/signflip_seed{seed}.jsonl",
                )
            ],
            "--bootstrap-samples",
            "10000",
            ...
            "0.95",
            "--output",
            ".confirmatory/v9_local/evidence/sealed_comparison.json",
        ],
        "wandb": {
            "entity": WANDB_ENTITY,
...
def _verify_source_snapshot(inventory: dict[str, Any]) -> None:
    ...
    with zipfile.ZipFile(path) as archive:
        if archive.namelist() != sorted(inventory["files"]):
            raise ProtocolError("V9-local source snapshot file list changed")
        for name, identity in inventory["files"].items():
            if hashlib.sha256(archive.read(name)).hexdigest() != identity["sha256"]:
                raise ProtocolError(f"V9-local source snapshot changed: {name}")
...
def _registration(
    *,
    ...
    if (
        ...
        != "j-lens-rl-confirmatory-v9-local-profanity-registration-draft"
        ...
        != "3caf91c90ff5dd54170ebaab658494d643fde4eb681b7fe18ddfc26b79e82dc3"
        or draft.get("fixed_input_sha256", {}).get("v8_terminal_closeout")
        != V8_CLOSEOUT_SHA256
    ):
        raise ProtocolError("V9-local committed registration draft changed")
    return {
        "schema_version": 1,
        "document_type": REGISTRATION_PROTOCOL,
        "protocol": PROTOCOL,
        "status": "prospectively_registered_fresh_v9_after_committed_v8_closeout",
        "git_commit": commit,
        ...
        "v8_terminal_closeout_sha256": amendment["v8_terminal_closeout_sha256"],
        "recipe_lock_sha256": recipe_sha,
        "code_identity": code,
...
def prepare() -> dict[str, Any]:
    ...
    if STATE_DIR.exists():
        raise ProtocolError(f"{STATE_DIR} already exists; V9-local preparation is immutable")
    recipe = _validate_recipe(read_json(RECIPE_SOURCE))
    amendment, closeout_path, v8_closeout_path = _amendment()
    if closeout_path is None or v8_closeout_path is None:
        raise ProtocolError(
            "V9-local launch is disabled until the committed V7 and V8 closeouts exist"
        )
    ...
    if (
        ...
        raise ProtocolError("V9-local materialized manifests differ byte-for-byte")
    ...
    shutil.copyfile(v8_closeout_path, REPRO_DIR / "v8_terminal_closeout.json")
    shutil.copytree(
        V8_TERMINAL_EVIDENCE_SOURCE,
        REPRO_DIR / "v8_terminal_evidence",
    )
    shutil.copyfile(
        REPO / "protocol_archive/v7_profanity_terminal_closeout.json",
...
def _verify_runtime_worktree(state: dict[str, Any]) -> None:
    if not RUNTIME_WORKTREE.is_dir():
        raise ProtocolError("dedicated V9-local runtime worktree is missing")
    if _runtime_git("rev-parse", "HEAD") != state["git_commit"]:
        raise ProtocolError("V9-local runtime worktree moved from registered commit")
    if _runtime_git("status", "--porcelain=v1", "--untracked-files=all"):
        raise ProtocolError("V9-local runtime worktree became dirty")
    link = RUNTIME_WORKTREE / ".confirmatory" / "v9_local"
    if not link.is_symlink() or link.resolve() != STATE_DIR.resolve():
        raise ProtocolError("V9-local runtime state link changed")
...
def load_and_verify_state(*, require_launch: bool = False) -> dict[str, Any]:
    if not STATE_PATH.is_file():
        raise ProtocolError("V9-local state has not been prepared")
    ...
    if (
        ...
        raise ProtocolError("V9-local registered recipe/source identity changed")
    ...
    if current_prepared != expected_prepared:
        raise ProtocolError("V9-local prepared-file inventory changed")
    if (
        ...
        raise ProtocolError("V9-local frozen data/artifact bytes changed")
    recipe = read_json(REPRO_DIR / "recipe_lock.json")
    schema = _metric_schema(recipe)
    if read_json(REPRO_DIR / "metric_schema.json") != schema:
        raise ProtocolError("V9-local metric schema changed")
    configs = _materialize_configs(
        recipe,
    ...
    for label, expected in configs.items():
        if read_json(CONFIG_DIR / f"{label}.json") != expected:
            raise ProtocolError(f"generated V9-local config changed: {label}")
    if read_json(REPRO_DIR / "launch_plan.json") != _launch_plan(configs):
        raise ProtocolError("V9-local launch plan changed")
    ...
    if require_launch:
        ...
        _validate_v8_closeout(
            amendment,
            REPRO_DIR / "v8_terminal_closeout.json",
            REPRO_DIR / "v8_terminal_evidence",
        )
    return state
...
def _verify_process_command(process: Any, config_path: Path) -> None:
    ...
    if (
        ...
        or len(argv) != 5
        or argv[0] != "scripts/v9_local_train.py"
        or argv[1] != "--config"
...
def verify_completed_runs(conditions: Sequence[str] = CONDITIONS) -> dict[str, Any]:
    if not conditions or any(condition not in CONDITIONS for condition in conditions):
        raise ProtocolError(f"invalid V9-local conditions: {conditions}")
    runs: dict[str, Any] = {}
    ...
    if len(source_trees) != 1:
        raise ProtocolError("V9-local runs used different source trees")
...
def compute_curve_gate(*, write_result: bool = True) -> dict[str, Any]:
    if write_result and (CURVE_PATH.exists() or CURVE_PLOT_PATH.exists()):
        raise ProtocolError("refusing to overwrite the one V9-local curve decision")
    state = load_and_verify_state(require_launch=True)
    ...
    if write_result:
        ...
        axis.set_title("V9-local profanity: exposed development consistency curve")
        axis.grid(alpha=0.2)
        axis.legend()
        figure.tight_layout()
...
def verify_curve_gate(*, require_pass: bool = True) -> dict[str, Any]:
    if not CURVE_PATH.is_file() or not CURVE_PLOT_PATH.is_file():
        raise ProtocolError("V9-local curve decision has not been recorded")
    ...
    for key in (
        ...
        if stored.get(key) != recomputed.get(key):
            raise ProtocolError("stored V9-local curve decision changed")
    if stored.get("curve_plot_sha256") != sha256_file(CURVE_PLOT_PATH):
        raise ProtocolError("V9-local curve plot changed")
    if require_pass and stored.get("passed") is not True:
        raise ProtocolError("V9-local exposed-development consistency gate failed")
    return stored


def unlock_final() -> dict[str, Any]:
    if UNLOCK_PATH.exists() or COMPLETED_RUNS_PATH.exists():
        raise ProtocolError("V9-local final unlock already exists")
    state = load_and_verify_state(require_launch=True)
...
def verify_unlock() -> dict[str, Any]:
    if not UNLOCK_PATH.is_file() or not COMPLETED_RUNS_PATH.is_file():
        raise ProtocolError("V9-local sealed final is not unlocked")
    ...
    if (
        ...
        raise ProtocolError("V9-local final unlock changed")
...
def begin_final_collection(collection_id: str) -> dict[str, Any]:
    ...
    if COLLECTION_PATH.exists() or EVAL_DIR.exists() or COMPARISON_PATH.exists():
        raise ProtocolError("V9-local sealed final was already claimed or opened")
    state = load_and_verify_state(require_launch=True)
    marker = {
        "protocol": PROTOCOL,
        "git_commit": state["git_commit"],
...
def verify_final_collection(collection_id: str | None = None) -> dict[str, Any]:
    ...
    if not COLLECTION_PATH.is_file():
        raise ProtocolError("V9-local final collection has not been claimed")
    ...
    if (
        ...
        raise ProtocolError("V9-local final collection marker changed")
    return marker
...
def verify_design() -> dict[str, Any]:
    recipe = _validate_recipe(read_json(RECIPE_SOURCE))
    source_inputs = _source_inputs()
    amendment, closeout, v8_closeout = _amendment()
    draft = read_json(REGISTRATION_DRAFT_SOURCE)
    if draft.get("launch_gate", {}).get("enabled") is not False:
        raise ProtocolError("V9-local draft unexpectedly enables launch")
    return {
        "protocol": PROTOCOL,
        "status": (
            "design_valid_launch_authorized_pending_clean_committed_pushed_source"
            if closeout is not None and v8_closeout is not None
