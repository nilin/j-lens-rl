#!/usr/bin/env python3
"""Fail-closed serial local runner for the registered V8 profanity replication."""
    ...
    with protocol.LOCAL_GPU_LOCK.open("a+") as handle:
        try:
            ...
            raise protocol.ProtocolError(
                "another process holds the registered V8-local GPU lock"
            ) from error
        handle.seek(0)
        handle.truncate()
        handle.write(
...
def _claim() -> dict[str, Any]:
    ...
    if protocol.CLAIM_PATH.is_file():
        ...
        if (
            ...
            raise protocol.ProtocolError("existing V8-local claim is not safely resumable")
        return claim
    if any(
        path.exists()
        for path in (
        ...
        raise protocol.ProtocolError("V8-local output exists before the one attempt claim")
    claim = {
        "schema_version": 1,
        "protocol": protocol.PROTOCOL,
