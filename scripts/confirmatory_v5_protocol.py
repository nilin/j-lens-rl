#!/usr/bin/env python3
"""Prepare and guard the emotional-word V5 confirmatory experiment.

V5 is intentionally inert until two committed records exist:

* ``protocol_archive/v5_emotional_selected_recipe.json`` freezes one fully
  resolved emotional-only training recipe and byte-identifies its artifacts.
* ``protocol_archive/v5_emotional_registration.json`` byte-pins that lock,
  the runner, the untouched data split, and four explicit curve nodes.

The protocol never chooses a word, sign, recipe, checkpoint, or curve node.
It only verifies the registered choices, derives eight matched sign-flipped
controls mechanically, and keeps all V5 state below ``.confirmatory/v5`` so
the completed V4 state remains untouched.
"""
...
STATE_DIR = REPO / ".confirmatory" / "v5"
...
REGISTRATION_PATH = REPO / "protocol_archive" / "v5_emotional_registration.json"
DEFAULT_RECIPE_LOCK_PATH = (
    REPO / "protocol_archive" / "v5_emotional_selected_recipe.json"
)
EMOTIONAL_DECISION_PATH = REPO / "protocol_archive" / "emotional_only_decision.json"
SUPERSEDED_V5_PATH = REPO / "protocol_archive" / "v5_preregistration.json"
V4_CLOSEOUT_PATH = REPO / "protocol_archive" / "v4_closeout.json"
CORRELATION_AMENDMENT4_PATH = (
    REPO / "protocol_archive" / "word_correlation_v1_amendment4.json"
)
...
REGISTRATION_PROTOCOL = "j-lens-rl-confirmatory-v5-emotional-registration-v1"
RECIPE_LOCK_PROTOCOL = "j-lens-rl-emotional-selected-recipe-lock-v1"
PROTOCOL = "j-lens-rl-confirmatory-v5-emotional"
...
SUPERSEDED_V5_SHA256 = "08dac96773ada5845815304bc256eebf6e7fb54b7a2b2cc95396851f690f0f99"
...
CONCURRENT_EXPERIMENT_FIREWALL = {
    "word_correlation_attempt4": {
        "amendment_path": "protocol_archive/word_correlation_v1_amendment4.json",
        "amendment_sha256": CORRELATION_AMENDMENT4_SHA256,
        "launch_receipt_path": (
            "protocol_archive/word_correlation_attempt4_launch_receipt.json"
        ),
        "launch_receipt_sha256": CORRELATION_ATTEMPT4_LAUNCH_SHA256,
        "status_at_v5_freeze": (
            "launched before V5 freeze; no correlation outcome inspected or used"
        ),
        "selection_eligibility": (
            "ineligible to change this V5 joy recipe; selection source is solely the "
            "archived emotional single-word joy seed-167 screen"
        ),
    }
}

JOY_SOURCE_CONFIG_SHA256 = "1f676febead8b43216f18ca0a6415ab36f3b67d9624c94d1524def000671f885"
JOY_CALIBRATION_SHA256 = "71979e2c36b10d759fc92d5b16e780ef699a34f1b7ea890a8c5f00257c8e2021"
JOY_SELECTION_EVIDENCE_SHA256 = {
    "resolved_config": JOY_SOURCE_CONFIG_SHA256,
    "calibration": JOY_CALIBRATION_SHA256,
    "run_manifest": "e30a6cf323f9875b69cd91debc10fe02b9817016f4141b3a40a22b7e99f55657",
    "screen_result": "cb41b80914dff5798ab30bdeb2840fc939486cdc3abf50092efd87b9508b379d",
    "validation_history": "c49d90a6449bbe4f132337fef478f7a228a18aec83bfb07c8b0faa2281016cec",
    "log_history": "4fb3ec710a5967252336b3c5e8e710e1d87edb9c8742c9f9d86d659aa40808fa",
    "final_adapter": "3b6847a0c5720df381edd023769b81e89390941b3cb244c60b88cdb5a8989e55",
}
JOY_DECLARED_TRANSFORMATIONS = {
    "updates": {"from": 25, "to": 6},
    "save_every": {"from": 25, "to": 6},
    "validation_steps": {
        "from": [2, 4, 6, 10, 15, 20, 25],
        "to": [2, 4, 6],
    },
    "calibration_path": {
        "from": "/single_word_screen/artifacts/joy_calibration.json",
        "to": "source_calibration.path",
    },
    "generated_run_identity": {
        "seed": "replace exploratory seed 167 with registered seeds 168..175",
        "output_dir": "derive from registered V5 condition and seed",
        "run_name_and_wandb": "derive from registered W&B prefix and condition",
        "data_paths": "replace with frozen V5 curve/exclusion/final manifests",
        "artifact_paths": "replace with byte-identical V5 frozen artifacts",
        "control": "negate only the registered treatment component weights",
    },
}

SPLIT_REGISTRATION = {
    "allocation_salt": V5_ALLOCATION_SALT,
    "source_parent": {
        "size": 1700,
        "manifest_sha256": V4_PARENT_SHA256,
        "sorted_set_sha256": V4_PARENT_SET_SHA256,
    },
    "curve": {
        "size": 400,
        "manifest_sha256": V5_CURVE_SHA256,
        "sorted_set_sha256": V5_CURVE_SET_SHA256,
    },
    "sealed_final": {
        "size": 1300,
...
ANALYSIS_REGISTRATION = {
    "primary_estimand": (
        "paired difference-in-differences: (joy-treatment minus base) minus "
        "(signflip-control minus base), matched by seed and sealed item"
    ),
    "secondary_estimand": "paired joy-treatment minus base across seeds and items",
...
def require_clean_worktree() -> str:
    ...
    if status:
        raise ProtocolError(
            "V5 confirmatory work requires a clean committed tree; git status is:\n"
            + status
        )
...
def observed_joy_history_scalar_series(target_label: str) -> dict[str, dict[str, Any]]:
    """Map every scalar key observed in the byte-pinned selected joy history."""
...
def metric_schema(
    ...
    return {
        ...
        "observed_history_scalar_series": observed_joy_history_scalar_series(label),
        "other_trainer_fields": {
            "storage": "all 38 scalar keys from the selected joy history are explicitly mapped above and preserved verbatim in log_history.json",
            "step_axis": "optimizer_update",
            "wandb_prefix": "train/",
        },
    }
...
def _expected_execution_hashes() -> dict[str, str]:
    paths = {
        "protocol_sha256": REPO / "scripts" / "confirmatory_v5_protocol.py",
        "modal_runner_sha256": REPO / "modal_confirmatory_v5.py",
        "shell_launcher_sha256": REPO / "run_confirmatory_v5.sh",
...
def registration_template() -> dict[str, Any]:
    """Return an intentionally incomplete template; it cannot launch V5."""
    return {
        "protocol": REGISTRATION_PROTOCOL,
        "claim_protocol": PROTOCOL,
        "frozen_at_utc": None,
        "supersedes": {
            "v5_preregistration_sha256": SUPERSEDED_V5_SHA256,
            "emotional_only_decision_sha256": EMOTIONAL_DECISION_SHA256,
            "v4_closeout_sha256": V4_CLOSEOUT_SHA256,
        },
        "concurrent_experiment_firewall": CONCURRENT_EXPERIMENT_FIREWALL,
    ...
    return {
        ...
        "selection_provenance": {
            "source_resolved_config": {
                "path": None,
                "sha256": JOY_SOURCE_CONFIG_SHA256,
            },
            "source_calibration": {
                "path": None,
                "sha256": JOY_CALIBRATION_SHA256,
            },
            "source_closeout": {"path": None, "sha256": None},
            "source_evidence_sha256": JOY_SELECTION_EVIDENCE_SHA256,
            "declared_transformations": JOY_DECLARED_TRANSFORMATIONS,
        },
...
def _load_registration() -> tuple[dict[str, Any], str]:
    if not REGISTRATION_PATH.is_file():
        raise ProtocolError(
            f"final emotional V5 registration is absent: {REGISTRATION_PATH}; "
            "freeze the selected recipe and explicit curve nodes before prepare"
        )
    registration = json.loads(REGISTRATION_PATH.read_text())
    if not isinstance(registration, dict):
        raise ProtocolError("V5 registration must be a JSON object")
...
def _validate_archive_lineage(registration: dict[str, Any]) -> None:
    expected_files = {
        ...
        SUPERSEDED_V5_PATH: SUPERSEDED_V5_SHA256,
    ...
    if registration.get("supersedes") != expected_supersedes:
        raise ProtocolError("registration does not pin the emotional-only lineage")
    if registration.get("concurrent_experiment_firewall") != (
        CONCURRENT_EXPERIMENT_FIREWALL
    ):
...
def _validate_registration_shape(registration: dict[str, Any]) -> None:
    if (
        ...
        raise ProtocolError("wrong V5 registration protocol")
    ...
    if registration.get("split") != SPLIT_REGISTRATION:
        raise ProtocolError("registration changed the untouched V5 data split")
    if registration.get("seeds") != list(SEEDS):
        raise ProtocolError("registration must use seeds 168 through 175 exactly")
    ...
    if not isinstance(curve, dict) or curve.get("criterion") != CURVE_CRITERION:
    ...
    for key in ("entity", "project", "group", "run_prefix"):
        if not isinstance(wandb.get(key), str) or not wandb[key]:
            raise ProtocolError(f"registration wandb.{key} must be a non-empty string")
    if wandb["project"] != "j-lens-rl":
        raise ProtocolError("registration W&B project must be exactly 'j-lens-rl'")
    if not re.fullmatch(r"[A-Za-z0-9_.-]+", wandb["run_prefix"]):
        raise ProtocolError("wandb.run_prefix contains unsupported characters")
    tags = wandb.get("tags")
    if (
        ...
        raise ProtocolError("registration wandb.tags must be non-empty strings")
    if registration.get("outcome_status_at_freeze") != (
        "not launched and no V5 outcome inspected"
    ):
        raise ProtocolError("registration must freeze before any V5 outcome inspection")
...
def _load_tracked_pinned_json(
    ...
    *,
    ...
    if expected_sha256 is not None and digest != expected_sha256:
        raise ProtocolError(f"{label} is not the exact selected joy input")
...
def expected_selected_joy_recipe(
    ...
    required_source = {
        ...
        "calibration_sha256": JOY_CALIBRATION_SHA256,
    ...
    if any(source.get(key) != value for key, value in required_source.items()):
        raise ProtocolError("selected source is not the exact joy U5 replay recipe")
    result = dict(source)
    result.update(
        {
            "updates": 6,
...
def _validate_joy_selection_provenance(
    ...
    if not isinstance(value, dict) or set(value) != {
        ...
        raise ProtocolError("selected joy provenance has unexpected fields")
    if value.get("source_evidence_sha256") != JOY_SELECTION_EVIDENCE_SHA256:
        raise ProtocolError("selected joy evidence identities changed")
    if value.get("declared_transformations") != JOY_DECLARED_TRANSFORMATIONS:
        raise ProtocolError("selected joy transformations changed")
    config_path, source = _load_tracked_pinned_json(
        value["source_resolved_config"],
        "selection_provenance.source_resolved_config",
        expected_sha256=JOY_SOURCE_CONFIG_SHA256,
    )
    calibration_path, calibration = _load_tracked_pinned_json(
        value["source_calibration"],
        "selection_provenance.source_calibration",
        expected_sha256=JOY_CALIBRATION_SHA256,
    )
    ...
    if any(
        ...
        for digest in JOY_SELECTION_EVIDENCE_SHA256.values()
    ) or "joy" not in serialized_closeout.lower():
        raise ProtocolError("source closeout does not bind the exact joy replay evidence")
    if calibration.get("target_words") != ["joy"] or (
        calibration.get("lens_sha256")
        != "178a9671cbf41882135807bde59b828e36c6f8f98b32c809ea3346860aad10dc"
    ):
        raise ProtocolError("committed calibration is not the exact joy calibration")
    expected = expected_selected_joy_recipe(
        source, calibration_path.relative_to(REPO).as_posix()
    )
    if recipe != expected:
        raise ProtocolError(
            "selected recipe is not the exact declared 25-to-6 joy transformation"
        )
    return {
        "source_resolved_config": {
...
def _load_recipe_lock(
    ...
    _validate_joy_selection_provenance(lock.get("selection_provenance"), recipe)
...
def validate_artifacts(recipe: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raise ProtocolError("run V5 preparation with the project virtualenv") from error
...
def _run_label(condition: str, seed: int) -> str:
    if condition not in REQUIRED_CONDITIONS or seed not in SEEDS:
        raise ProtocolError("run label is outside the frozen V5 protocol")
...
def _wandb_identity(
    ...
    prefix = wandb["run_prefix"]
    run_id = f"{prefix}-{registration_sha256[:12]}-{label}"
    return {
...
def generated_configs(
    ...
    base.update(
        {
            "validation_indices_path": ".confirmatory/v5/manifests/curve_indices.json",
            "reserved_train_indices_path": ".confirmatory/v5/manifests/train_exclusions.json",
            "validation_examples": 400,
            "lens_path": ".confirmatory/v5/frozen_artifacts/lens.pt",
            "calibration_path": ".confirmatory/v5/frozen_artifacts/calibration.json",
            ...
            "metric_schema_path": ".confirmatory/v5/reproducibility/metric_schema.json",
    ...
    for seed in SEEDS:
        ...
        treatment.update(
            {
                "seed": seed,
                "output_dir": f".confirmatory/v5/runs/{treatment_label}",
                **_wandb_identity(registration, registration_sha256, treatment_label),
                "registered_command": [
                    ...
                    "jlens_rl.train",
                    "--config",
                    f".confirmatory/v5/configs/{treatment_label}.json",
                    "--wandb-mode",
                    "online",
                ],
            }
        ...
        control.update(
            {
                ...
                "output_dir": f".confirmatory/v5/runs/{control_label}",
                **_wandb_identity(registration, registration_sha256, control_label),
                "registered_command": [
                    ...
                    "jlens_rl.train",
                    "--config",
                    f".confirmatory/v5/configs/{control_label}.json",
                    "--wandb-mode",
                    "online",
                ],
            }
    ...
    sealed.update(
        {
            ...
            "validation_examples": 1300,
            "min_new_tokens": 0,
            "max_new_tokens": 256,
            "output_dir": ".confirmatory/v5/evaluation_config_unused",
...
def _verify_source_manifests() -> tuple[list[int], list[int], list[int]]:
    ...
    for path, digest in expected.items():
        if not path.is_file() or sha256_file(path) != digest:
            raise ProtocolError(f"V5 source manifest is absent or changed: {path}")
    parent = load_indices(V4_PARENT_PATH)
    reserve = load_indices(V4_RESERVE_PATH)
    exclusions = load_indices(V4_TRAIN_EXCLUSIONS_PATH)
    if canonical_sha256(sorted(parent)) != V4_PARENT_SET_SHA256:
        raise ProtocolError("V4 sealed parent set identity changed")
    if len(reserve) != 64 or set(parent) & set(reserve):
        raise ProtocolError("untouched reserve is malformed or overlaps the V5 parent")
    if not (set(parent) | set(reserve)) <= set(exclusions):
        raise ProtocolError("training exclusions no longer protect all V5 outcomes")
    return parent, reserve, exclusions


def _verify_v5_allocations(curve: Sequence[int], final: Sequence[int]) -> None:
    if (
        len(curve) != 400
        or len(final) != 1300
        or set(curve) & set(final)
        or sha256_file(MANIFEST_DIR / "curve_indices.json") != V5_CURVE_SHA256
        or sha256_file(MANIFEST_DIR / "sealed_final_indices.json") != V5_FINAL_SHA256
        or canonical_sha256(sorted(curve)) != V5_CURVE_SET_SHA256
        or canonical_sha256(sorted(final)) != V5_FINAL_SET_SHA256
    ):
        raise ProtocolError("prepared V5 curve/final allocation changed")
...
def _validate_frozen_artifacts(
    ...
    try:
        ...
        raise ProtocolError("run V5 verification with the project environment") from error
    lens = JacobianLens.load(str(paths["lens"]))
    layers = [int(layer) for layer in lens.source_layers]
    tokenizer = AutoTokenizer.from_pretrained(MODEL_NAME, revision=MODEL_REVISION)
    token_ids = single_token_ids(tokenizer, words)
...
def _launch_plan(
    ...
    reproduction_training = {
        label: [
            ...
            "jlens_rl.train",
            "--config",
            f".confirmatory/v5/configs/{label}.json",
            "--reproduction-replay",
            "--output-dir",
    ...
    for label in FINAL_LABELS:
        if label == "base":
            experiment = ".confirmatory/v5/configs/jlens_seed168.json"
            adapter: list[str] = []
        else:
            experiment = f".confirmatory/v5/configs/{label}.json"
            adapter = ["--adapter", f".confirmatory/v5/runs/{label}/final"]
        evaluations[label] = [
            "python",
            "-m",
            "jlens_rl.eval",
            "--config",
            ".confirmatory/v5/configs/sealed_eval.json",
            "--experiment-config",
            experiment,
            "--indices-manifest",
            ".confirmatory/v5/manifests/sealed_final_indices.json",
            "--output-jsonl",
            f".confirmatory/v5/evals/{label}.jsonl",
            "--run-label",
            label,
            "--batch-size",
            "64",
    ...
    return {
        ...
        "analysis_command": [
            ...
            "--base-jsonl",
            ".confirmatory/v5/evals/base.jsonl",
            *[
                item
                for seed in SEEDS
                for item in (
                    "--adapter-jsonl",
                    f".confirmatory/v5/evals/jlens_seed{seed}.jsonl",
                )
            ],
            *[
                item
                ...
                for item in (
                    "--control-jsonl",
                    f".confirmatory/v5/evals/signflip_seed{seed}.jsonl",
                )
            ],
            "--bootstrap-samples",
            "10000",
            ...
            "0.95",
            "--output",
            ".confirmatory/v5/evidence/sealed_comparison.json",
        ],
        "wandb": registration["wandb"],
    }
...
def _verify_reproducibility_files(
    ...
    if actual_names != expected_names:
        raise ProtocolError(
            f"V5 reproducibility inventory changed: {sorted(actual_names)}"
        )
    if (
    ...
    if json.loads((REPRODUCIBILITY_DIR / "metric_schema.json").read_text()) != (
        ...
        raise ProtocolError("V5 metric schema changed")
    if json.loads((REPRODUCIBILITY_DIR / "launch_plan.json").read_text()) != (
        _launch_plan(registration, configs)
    ):
        raise ProtocolError("V5 replay command plan changed")
...
def prepare() -> None:
    ...
    if STATE_DIR.exists():
        raise ProtocolError(f"{STATE_DIR} already exists; V5 preparation is immutable")
    ...
    curve, final = allocate_v5(parent)
    ...
    _verify_v5_allocations(curve, final)
    if sha256_file(MANIFEST_DIR / "future_reserve_indices.json") != RESERVE_SHA256:
        raise ProtocolError("V5 changed the untouched reserve bytes")
    if sha256_file(MANIFEST_DIR / "train_exclusions.json") != TRAIN_EXCLUSIONS_SHA256:
        raise ProtocolError("V5 changed the protected training exclusions")
...
def _verify_prepared_manifests() -> None:
    expected = {
        "curve_indices.json": V5_CURVE_SHA256,
        "sealed_final_indices.json": V5_FINAL_SHA256,
    ...
    for name, digest in expected.items():
        ...
        if not path.is_file() or sha256_file(path) != digest:
            raise ProtocolError(f"prepared V5 manifest changed: {name}")
    ...
    if (
        ...
        raise ProtocolError("prepared V5 manifests overlap or escape training exclusions")


def load_and_verify_state() -> dict[str, Any]:
    commit = require_clean_worktree()
    if not STATE_PATH.is_file():
        raise ProtocolError("V5 is not prepared; run prepare only after final registration")
    ...
    if any(state.get(key) != value for key, value in expected_state.items()):
        raise ProtocolError("prepared state does not match the final V5 registration")
    ...
    if (
        ...
        raise ProtocolError("prepared V5 reproducibility hashes changed")
    return state
...
def training_behavior_summary(path: Path, config: dict[str, Any]) -> dict[str, Any]:
    ...
    expected_scalar_keys = set(observed_joy_history_scalar_series(label))
...
def verify_completed_runs(
    ...
    if not conditions or any(condition not in REQUIRED_CONDITIONS for condition in conditions):
        raise ProtocolError(f"invalid conditions for V5 run verification: {conditions}")
    ...
    for condition in conditions:
        for seed in SEEDS:
            ...
            if matched_source_tree is None:
                ...
                raise ProtocolError("V5 required runs used different source trees")
            ...
            if (
                ...
                != "j-lens-rl-confirmatory-v5-emotional-image-v1"
            ):
            ...
            if matched_runtime is None:
                ...
                raise ProtocolError("matched V5 runs used different numerical runtimes")
            expected_wandb = {
...
def completed_run_artifact_manifest() -> dict[str, Any]:
    ...
    if len(source_trees) != 1:
        raise ProtocolError("completed V5 runs do not share one source-tree identity")
    return {
...
def _render_curve_plot(
    ...
    axis.set_title("Emotional-word V5: fixed observational curve")
    axis.grid(alpha=0.2)
    axis.legend()
    figure.tight_layout()
...
def compute_curve_gate(write_result: bool = True) -> dict[str, Any]:
    if write_result and (CURVE_GATE_PATH.exists() or CURVE_PLOT_PATH.exists()):
        raise ProtocolError("refusing to overwrite the recorded V5 curve gate")
    state = load_and_verify_state()
...
def verify_curve_gate() -> dict[str, Any]:
    ...
    if not CURVE_GATE_PATH.is_file() or not CURVE_PLOT_PATH.is_file():
        raise ProtocolError("the V5 semantic curve gate has not been recorded")
    ...
    if any(recomputed.get(field) != stored.get(field) for field in fields):
        raise ProtocolError("stored V5 curve gate no longer matches semantic histories")
    ...
    if (
        ...
        raise ProtocolError("stored V5 curve plot is missing or changed")
    if stored.get("passed") is not True:
        raise ProtocolError("the registered V5 semantic curve gate did not pass")
    return stored


def unlock_final() -> None:
    if UNLOCK_PATH.exists():
        raise ProtocolError("V5 sealed-final unlock already exists")
...
def verify_unlock() -> dict[str, Any]:
    ...
    if not UNLOCK_PATH.is_file() or not COMPLETED_RUNS_PATH.is_file():
        raise ProtocolError("V5 sealed final evaluation is not unlocked")
    ...
    if (
        ...
        raise ProtocolError("V5 unlock or completed-run artifacts changed")
    gate = verify_curve_gate()
    if marker.get("curve_gate_sha256") != sha256_file(CURVE_GATE_PATH) or not gate["passed"]:
        raise ProtocolError("V5 curve gate changed after unlock")
    return marker
...
def begin_final_collection(collection_id: str) -> dict[str, Any]:
    ...
    if FINAL_COLLECTION_PATH.exists():
        raise ProtocolError("the one immutable V5 final collection is already claimed")
...
def verify_final_collection(collection_id: str | None = None) -> dict[str, Any]:
    ...
    if any(marker.get(key) != value for key, value in expected.items()):
        raise ProtocolError("immutable V5 final collection marker changed")
    marker_id = marker.get("collection_id")
    if not isinstance(marker_id, str) or not re.fullmatch(r"[a-f0-9]{32}", marker_id):
        raise ProtocolError("invalid immutable V5 final collection ID")
...
def _evaluation_role(label: str) -> tuple[str, int, bool]:
    ...
    if match is None:
        raise ProtocolError(f"invalid V5 evaluation label: {label!r}")
    condition, seed_text = match.groups()
    seed = int(seed_text)
    if seed not in SEEDS:
        raise ProtocolError(f"V5 evaluation label has an unregistered seed: {label!r}")
...
def verify_evaluation_jsonl(path: Path, label: str) -> None:
    ...
    if path.resolve() != expected_path.resolve():
        raise ProtocolError(f"V5 evaluation {label!r} must use {expected_path}")
    ...
    if len(records) != 1300:
        raise ProtocolError(f"{path} has {len(records)} rows; expected 1,300")
    ...
    if provenance.get("run_label") != label or provenance.get("evaluation_seed") != 0:
        raise ProtocolError(f"{path} is not bound to V5 role {label!r}")
    adapter = provenance.get("adapter")
    if is_base:
        if adapter is not None:
            raise ProtocolError("V5 base evaluation unexpectedly used an adapter")
    ...
    if (
        ...
        or manifest_identity.get("count") != 1300
    ):
...
def final_evaluation_hashes() -> dict[str, str]:
    ...
    if actual_names != expected_names:
        raise ProtocolError(
            "V5 final collection must contain exactly the 17 registered JSONL labels"
        )
    return {
...
def _current_bundle_inventory() -> dict[str, Any]:
    ...
    return {
        ...
        "interpretation": (
            "The automatic durable archive contains this entire finalized V5 state. "
...
def write_evidence_bundle_inventory() -> dict[str, Any]:
    if BUNDLE_INVENTORY_PATH.exists():
        raise ProtocolError("refusing to overwrite V5 evidence-bundle inventory")
...
def verify_evidence_bundle_inventory() -> dict[str, Any]:
    if not BUNDLE_INVENTORY_PATH.is_file():
        raise ProtocolError("V5 evidence-bundle inventory is missing")
    stored = json.loads(BUNDLE_INVENTORY_PATH.read_text())
    if stored != _current_bundle_inventory():
        raise ProtocolError("V5 evidence bundle changed after finalization")
    return stored
...
def _write_closeout_candidate(records: dict[str, Any]) -> dict[str, Any]:
    if CLOSEOUT_CANDIDATE_PATH.exists():
        ...
        if existing.get("claim_id") != records["claim"].get("claim_id"):
            raise ProtocolError("compact V5 closeout belongs to another attempt")
        return existing
    state = json.loads(STATE_PATH.read_text())
    acceptance = (
...
def _write_durable_export_plan(claim_id: str) -> dict[str, Any]:
    archive_name = f"v5_emotional_evidence_{claim_id}.zip"
    ...
    payload = {
        ...
        "composition": (
            "deterministic ZIP of every finalized V5 file except exports/; includes "
    ...
    if EXPORT_PLAN_PATH.exists():
        ...
        if existing != payload:
            raise ProtocolError("durable V5 export plan changed during retry")
        return existing
...
def _write_durable_archive(plan: dict[str, Any]) -> dict[str, Any]:
    ...
    if receipt_path.exists():
        ...
        if (
            ...
            raise ProtocolError("durable V5 export receipt no longer verifies")
        return receipt
    ...
    try:
        with zipfile.ZipFile(archive_path) as archive:
            ...
            if archive.namelist() != expected_names:
                raise ProtocolError("durable V5 export entry inventory changed")
            for source, name in zip(files, expected_names, strict=True):
                ...
                if archived_sha256 != sha256_file(source):
                    raise ProtocolError(
                        f"durable V5 export entry bytes changed: {name}"
                    )
    except zipfile.BadZipFile as error:
        raise ProtocolError("durable V5 export is not a valid ZIP") from error
    receipt = {
        "schema_version": 1,
...
def final_report() -> dict[str, Any]:
    ...
    if not SEALED_COMPARISON_PATH.is_file():
        raise ProtocolError("collect all 17 V5 evaluations before reporting")
    ...
    if comparison != _recompute_final_comparison():
        raise ProtocolError("V5 sealed comparison does not match frozen evaluations")
    ...
    try:
        if args.command == "registration-template":
            ...
            if not result["passed"]:
                raise ProtocolError("registered V5 curve gate failed")
        elif args.command == "verify-curve":
            print(json.dumps(verify_curve_gate(), indent=2, sort_keys=True))
        elif args.command == "verify-runs":
            verify_completed_runs()
            print("all 16 V5 matched fixed-horizon runs are verified")
        elif args.command == "verify-semantic":
            verify_completed_runs(("jlens",))
            print("all eight V5 emotional treatment runs are verified")
            ...
            print(f"V5 evaluation JSONL is complete and auditable: {args.path}")
            ...
            print(json.dumps(result, indent=2, sort_keys=True))
            if not result["passed"]:
                raise ProtocolError("V5 final acceptance criterion failed")
