"""Run one V9-local config and seal its completed offline W&B directory.

The shared trainer normally publishes a terminal W&B artifact online.  V9
...
def _observe_offline_identity(run: Any, identity: Any) -> dict[str, Any]:
    ...
    if observed["resume"] != "never" or observed["mode"] != "offline":
        raise RuntimeError("V9-local W&B mode/resume policy changed")
...
def publish_run_result_offline(
    *, output_dir: Path, result: dict[str, Any], enabled: bool
    ...
    if raw_wandb_dir is None:
        raise RuntimeError("V9-local requires a per-run WANDB_DIR")
...
def _atomic_embed_evidence_file(
    *, source: Path, queued: Path, expected_sha256: str
    ...
    temporary = queued.with_name(f".{queued.name}.v9-embed.tmp")
    ...
    try:
        try:
            ...
            if _pending_publication is None:
                _finish_wandb(exit_code=1, require_active=False)
                raise RuntimeError(
                    "registered V9-local training produced no terminal queue"
                )
