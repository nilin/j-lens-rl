...
APP_NAME = "j-lens-rl-development-v15-celebration-h5-20260715a"
VOLUME_NAME = "j-lens-rl-development-v15-celebration-h5-20260715a"
PROTOCOL = "j-lens-rl-development-v15-celebration-h5"
REGISTRATION_PATH = "protocol_archive/v15_celebration_h5_registration.json"
REGISTRATION_SHA256 = "35c78b2f60529e3db5ca8a7491109503267a9469d5a11ff5ca772831e91d67f8"
METRIC_SCHEMA_PATH = "protocol_archive/v15_celebration_h5_metric_schema.json"
METRIC_SCHEMA_SHA256 = "f869f4e64eea3dd772bb9c7b24495227d7b5764e6597097be12c40d1afd8df1b"
...
WANDB_GROUP = "dev-v15-celebration-u1-h5"
AGGREGATE_WANDB_ID = "dev-v15-celebration-aggregate"
RUNNER_PATH = "modal_v15_celebration_h5.py"
COMMON_CONFIG_PATH = "configs/v15_celebration_h5_common.json"
CONFIG_PATHS = {
    (condition, seed): (
        f"configs/v15_celebration_h5_{condition}_seed{seed}.json"
    )
    for condition in CONDITIONS
    for seed in SEEDS
}
WANDB_IDS = {
    (condition, seed): f"dev-v15-celebration-{condition}-seed{seed}"
    for condition in CONDITIONS
    for seed in SEEDS
}
LABELS = tuple(
...
EXPECTED_FILE_SHA256 = {
    ...
    COMMON_CONFIG_PATH: "a808d98e8693c2f2ffa1470e5d9c4d518b13e3d8bf912035ec247d48063e9adc",
    "configs/v15_celebration_h5_jlens_seed240.json": "707e700363bb3624d40518a65be831b5dd378928c1d4f83a2fad502978ad6b7c",
    "configs/v15_celebration_h5_jlens_seed241.json": "82c41c2bf46547b810e4b623cdc5a95a622a1df78da1859b4044f2176e98a5f1",
    "configs/v15_celebration_h5_jlens_seed242.json": "1865fc2d7a03bf4bb5e7451b40eb65637a0886e36e26d77ab7d7faf6273f260d",
    "configs/v15_celebration_h5_jlens_seed243.json": "42788ac51790318573539cdb5d5b2b8ad218dd7dd2c84c7eb99ed3fc76fb9e92",
    "configs/v15_celebration_h5_signflip_seed240.json": "6c96805436d733eb42f5e3230ab686beec3c5e62f7618163ebedef32d048a5c2",
    "configs/v15_celebration_h5_signflip_seed241.json": "edec4ea0483ff17097f1842eaccb202223fe8fdbc36b9e207d66d02b230e55cf",
    "configs/v15_celebration_h5_signflip_seed242.json": "a2e663fe9c319cff1066bc13a478bddc74bb098ff5afe7e90f98bc1109395d95",
    "configs/v15_celebration_h5_signflip_seed243.json": "7780811a890ad8138e5cebccd7c262bdab3c33ba3c2374f61162558f1f02b3a7",
...
def _label(condition: str, seed: int) -> str:
    if condition not in CONDITIONS or seed not in SEEDS:
        raise ValueError(f"unregistered V15 run: {condition=} {seed=}")
...
def validate_config(repository: Path, condition: str, seed: int) -> dict[str, Any]:
    ...
    changed_science = {
        key: {"seed195": source.get(key), "v15": config.get(key)}
        for key in SCIENCE_KEYS_UNCHANGED_FROM_SEED195
        if config.get(key) != source.get(key)
    }
    if changed_science:
        raise RuntimeError(f"V15 changed frozen celebration science: {changed_science}")
    expected_identity = {
        "seed": seed,
        "updates": 5,
        "eval_every": 1,
    ...
    if changed_identity:
        raise RuntimeError(f"V15 config identity changed: {changed_identity}")
    expected_components = (
    ...
    if config.get("score_components") != [dict(item) for item in expected_components]:
        raise RuntimeError(f"V15 {_label(condition, seed)} score changed")
...
parallel_image = (
    parallel_image.env(
        {
            "GIT_AUTHOR_NAME": "J-Lens V15 Runtime",
            "GIT_AUTHOR_EMAIL": "runtime@example.invalid",
            "GIT_COMMITTER_NAME": "J-Lens V15 Runtime",
            "GIT_COMMITTER_EMAIL": "runtime@example.invalid",
            "GIT_AUTHOR_DATE": "2000-01-01T00:00:00+00:00",
            "GIT_COMMITTER_DATE": "2000-01-01T00:00:00+00:00",
            "JLENS_REPOSITORY_ROOT": REMOTE_REPO.as_posix(),
            "JLENS_MODAL_IMAGE_SPEC": "j-lens-rl-development-v15-celebration-h5-l40s-v1",
    ...
    .run_commands(
        ...
        "git init -q",
        "git add -f .",
        "git commit -qm 'J-Lens V15 V14-selected development runtime'",
...
def _verify_runtime_files() -> dict[str, str]:
    ...
    if status:
        raise RuntimeError(f"V15 runtime Git worktree is dirty: {status}")
    ...
    if (
        ...
        raise RuntimeError("V15 registration classification or firewall changed")
    for condition in CONDITIONS:
        for seed in SEEDS:
...
def _read_claim(claim_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError("V15 attempt has no durable claim")
    value = json.loads(path.read_text())
    if (
        ...
        raise RuntimeError("V15 attempt claim changed")
    return value
...
def claim_attempt(claim_id: str, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(REMOTE_STATE.iterdir()):
        raise RuntimeError("registered V15 Volume is not fresh and empty")
    value = {
        "schema_version": 1,
        "protocol": PROTOCOL,
    ...
    while time.monotonic() < deadline:
        ...
        if path.is_file():
            ...
            if value.get("claim_id") == claim_id:
                return value
        time.sleep(1)
    raise RuntimeError("durable V15 launch receipt did not arrive")
...
def train_run(condition: str, seed: int, claim_id: str) -> dict[str, Any]:
    ...
    if completed.returncode:
        ...
        state_volume.commit()
        raise RuntimeError(f"V15 training failed closed for {label}")
...
def _write_aggregate_files(aggregate: Mapping[str, Any]) -> tuple[Path, Path, Path]:
    ...
    json_path = evidence / "v15_celebration_h5_aggregate.json"
    csv_path = evidence / "v15_celebration_h5_curve.csv"
    png_path = evidence / "v15_celebration_h5_curve.png"
...
def orchestrate(claim_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if receipt.get("root_call_id") != root_call_id:
            raise RuntimeError("V15 orchestrator lacks durable root authority")
        _read_claim(claim_id)
        ...
        if failures:
            raise RuntimeError(
                "V15 workers failed after all eight calls were drained: "
                + " | ".join(failures)
            )
...
def _local_preflight() -> dict[str, Any]:
    ...
    if status or head != pushed:
        raise RuntimeError("V15 launch requires an exact clean pushed main")
    for relative, expected in EXPECTED_FILE_SHA256.items():
    ...
    if modal_executable is None:
        raise RuntimeError("V15 preflight cannot locate the Modal CLI on PATH")
    ...
    if inventory:
        raise RuntimeError("V15 Volume must be fresh and empty")
