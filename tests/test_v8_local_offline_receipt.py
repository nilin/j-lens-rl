...
from scripts import v8_local_train as v8


RUN_ID = "confirm-v8-local-jlens_seed200"
...
def frozen_identity() -> dict:
    return {
        ...
        "entity": "entity",
        "project": "project",
        "run_name": "confirm-v8-local-jlens_seed200",
        "url": f"https://wandb.ai/entity/project/runs/{RUN_ID}",
        "group": "confirm-v8-local",
        "tags": ["confirmatory-v8-local", "offline-wandb"],
        "resume": "never",
    }
...
def build_case(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> SimpleNamespace:
    ...
    output = tmp_path / "runs" / "jlens_seed200"
    output.mkdir(parents=True)
    for number, name in enumerate(v8.EVIDENCE_FILE_NAMES):
        payload = (
    ...
    wandb_dir = tmp_path / "offline_wandb" / "jlens_seed200"
    ...
    monkeypatch.setattr(v8, "_pending_publication", None)
...
def queue(case: SimpleNamespace) -> dict:
    result = v8.publish_run_result_offline(
...
def finish_and_seal(case: SimpleNamespace) -> dict:
    ...
    assert v8._pending_publication is not None
    return v8.finalize_offline_terminal_receipt(v8._pending_publication)
...
def test_offline_receipt_is_written_only_after_finish_and_binds_exact_tree(
    ...
    queued = queue(case)
    receipt_path = case.output / v8.OFFLINE_RECEIPT_NAME
    assert not receipt_path.exists()
    assert queued["remote_artifact_claimed"] is False
    assert list(queued["queued_evidence_sha256"]) == list(v8.EVIDENCE_FILE_NAMES)
    assert [event for event in case.events if event.startswith("save:")] == [
        f"save:{name}" for name in v8.EVIDENCE_FILE_NAMES
    ]
    ...
    assert receipt["embedded_evidence_relative_paths"] == {
        name: f"files/{name}" for name in v8.EVIDENCE_FILE_NAMES
    }
    assert (
    ...
    assert receipt["offline_run_tree_sha256"] == v8._canonical_sha256(inventory)
    ...
    for name in v8.EVIDENCE_FILE_NAMES:
        ...
        assert not (case.files / f".{name}.v8-embed.tmp").exists()
    assert not (case.offline_root / v8.OFFLINE_RECEIPT_NAME).exists()
    assert not receipt_path.with_suffix(receipt_path.suffix + ".tmp").exists()
    assert (
        v8.validate_offline_terminal_receipt(
...
def test_main_finishes_before_atomic_receipt(
    ...
    case = build_case(tmp_path, monkeypatch)
    original_write = v8.train._write_json_atomic
    original_publisher = v8.train.publish_run_result_to_wandb
    ...
    monkeypatch.setattr(v8.train, "main", fake_training_main)
    monkeypatch.setattr(v8.train, "_write_json_atomic", recording_write)
    v8.main()
    ...
    assert v8.train.publish_run_result_to_wandb is original_publisher
    assert v8._pending_publication is None
    v8.validate_offline_terminal_receipt(case.output, case.identity, case.wandb_dir)
...
def test_queue_fails_closed_on_identity_or_mode_mismatch(
    ...
    with pytest.raises(RuntimeError, match=message):
        queue(case)
    assert not (case.output / v8.OFFLINE_RECEIPT_NAME).exists()
...
def test_multiple_matching_offline_roots_prevent_receipt(
    ...
    with pytest.raises(RuntimeError, match="exactly one"):
        v8.finalize_offline_terminal_receipt(v8._pending_publication)
    assert not (case.output / v8.OFFLINE_RECEIPT_NAME).exists()
...
def test_disabled_publication_has_no_tracking_side_effect(
    ...
    monkeypatch.setattr(v8, "_pending_publication", None)
    assert (
        v8.publish_run_result_offline(output_dir=Path("unused"), result={}, enabled=False)
        is None
    )
    assert v8._pending_publication is None
