"""Run the fresh five-update V14-selected celebration replication.
...
optimizer steps 0,1,2,3,4,5.  This runner is deliberately separate from all
closed and active predecessor attempts. It never mounts or reads a
...
APP_NAME = "j-lens-rl-development-v15b-celebration-h5-20260715a"
VOLUME_NAME = "j-lens-rl-development-v15b-celebration-h5-20260715a"
PROTOCOL = "j-lens-rl-development-v15b-celebration-h5"
REGISTRATION_PATH = "protocol_archive/v15b_celebration_h5_registration.json"
REGISTRATION_SHA256 = "21b016420fe8ed20db8b5ff40d0b7963eb8cf746e3880c9ba462ecc9dee6ac23"
METRIC_SCHEMA_PATH = "protocol_archive/v15b_celebration_h5_metric_schema.json"
METRIC_SCHEMA_SHA256 = "a49463cfba684e92b8a821bb13731497bc0f04369af9c9a38d6b44bdf936d600"
CANDIDATE_FREEZE_PATH = "protocol_archive/v15_celebration_h5_candidate_freeze.json"
CANDIDATE_FREEZE_SHA256 = "0e7d3fcb461687d8b7682709f271337b641936a4209099b19ad462729c3a5888"
...
SEEDS = (244, 245, 246, 247)
CONDITIONS = ("jlens", "signflip")
STEPS = tuple(range(6))
POST_BASELINE_STEPS = STEPS[1:]
DISPLAY_GATE_STEPS = (0, 3, 4, 5)
WANDB_ENTITY = "nilinabra-spare-time"
WANDB_PROJECT = "j-lens-rl"
WANDB_GROUP = "dev-v15b-celebration-u1-h5"
AGGREGATE_WANDB_ID = "dev-v15b-celebration-aggregate"
RUNNER_PATH = "modal_v15b_celebration_h5.py"
COMMON_CONFIG_PATH = "configs/v15b_celebration_h5_common.json"
CONFIG_PATHS = {
    (condition, seed): (
        f"configs/v15b_celebration_h5_{condition}_seed{seed}.json"
    )
    for condition in CONDITIONS
    for seed in SEEDS
}
WANDB_IDS = {
    (condition, seed): f"dev-v15b-celebration-{condition}-seed{seed}"
    for condition in CONDITIONS
    for seed in SEEDS
}
LABELS = tuple(
...
EXPECTED_FILE_SHA256 = {
    ...
    COMMON_CONFIG_PATH: "1588381abc79e806cfc66f6d329ae9f71b5cb66e48b077d3dd855c73334644be",
    "configs/v15b_celebration_h5_jlens_seed244.json": "d48bd98d3cf053c673b722a48fadc0f9ff837c413a8f133be968ecf4b4188820",
    "configs/v15b_celebration_h5_jlens_seed245.json": "467378d98204fb26c6ec550a25679c1f63eac420c3f90c688eddd3d84e31f0ed",
    "configs/v15b_celebration_h5_jlens_seed246.json": "3395d1af2a560e0267617db4f0af354853fdfa0c69bc53757c1218196368340d",
    "configs/v15b_celebration_h5_jlens_seed247.json": "3d5ade88f570f164bbe40436a51678296f662022f4741fd3170428bac76e4c65",
    "configs/v15b_celebration_h5_signflip_seed244.json": "8b4f07bbb46f7923913e4ee74d5687f65a1a5ceaf021617fca65882952a9af0b",
    "configs/v15b_celebration_h5_signflip_seed245.json": "188b3f01732c5a9ee40b5d215d962f317852cb0adc92c5959c04f23ec10693da",
    "configs/v15b_celebration_h5_signflip_seed246.json": "153e7c71881693a406e58821817724ec00ebc0f43f023b42aa0f3c96dce15aa3",
    "configs/v15b_celebration_h5_signflip_seed247.json": "5c7247bdb7b3b1797a5da8d794fe51cdeee1a83d768c91f4a06813521a888e12",
...
def _label(condition: str, seed: int) -> str:
    if condition not in CONDITIONS or seed not in SEEDS:
        raise ValueError(f"unregistered V15B run: {condition=} {seed=}")
...
def validate_config(repository: Path, condition: str, seed: int) -> dict[str, Any]:
    ...
    changed_science = {
        key: {"seed195": source.get(key), "v15b": config.get(key)}
        for key in SCIENCE_KEYS_UNCHANGED_FROM_SEED195
        if config.get(key) != source.get(key)
    }
    if changed_science:
        raise RuntimeError(f"V15B changed frozen celebration science: {changed_science}")
    expected_identity = {
        "seed": seed,
        "updates": 5,
        "eval_every": 1,
        ...
        "evidence_eligibility": (
            "development_only_adaptive_v14_horizon_no_protected_final"
        ),
    ...
    if changed_identity:
        raise RuntimeError(f"V15B config identity changed: {changed_identity}")
    expected_components = (
    ...
    if config.get("score_components") != [dict(item) for item in expected_components]:
        raise RuntimeError(f"V15B {_label(condition, seed)} score changed")
...
parallel_image = (
    parallel_image.env(
        {
            "GIT_AUTHOR_NAME": "J-Lens V15B Runtime",
            "GIT_AUTHOR_EMAIL": "runtime@example.invalid",
            "GIT_COMMITTER_NAME": "J-Lens V15B Runtime",
            "GIT_COMMITTER_EMAIL": "runtime@example.invalid",
            "GIT_AUTHOR_DATE": "2000-01-01T00:00:00+00:00",
            "GIT_COMMITTER_DATE": "2000-01-01T00:00:00+00:00",
            "JLENS_REPOSITORY_ROOT": REMOTE_REPO.as_posix(),
            "JLENS_MODAL_IMAGE_SPEC": "j-lens-rl-development-v15b-celebration-h5-l40s-v1",
    ...
    .run_commands(
        ...
        "git init -q",
        "git add -f .",
        "git commit -qm 'J-Lens V15B V14-selected development runtime'",
...
def _verify_runtime_files() -> dict[str, str]:
    ...
    if status:
        raise RuntimeError(f"V15B runtime Git worktree is dirty: {status}")
    ...
    if (
        ...
        raise RuntimeError("V15B registration classification or firewall changed")
    for condition in CONDITIONS:
        for seed in SEEDS:
...
def _read_claim(claim_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError("V15B attempt has no durable claim")
    value = json.loads(path.read_text())
    if (
        ...
        raise RuntimeError("V15B attempt claim changed")
    return value
...
def _validate_recoverable_dispatch(
    ...
    if (
        ...
        or status.get("stage") != "all_eight_spawned_capacity_queue_allowed"
        or status.get("max_parallel_gpus") != MAX_PARALLEL_GPUS
        or status.get("workspace_gpu_limit") != WORKSPACE_GPU_LIMIT
        ...
        raise RuntimeError("V15B existing dispatch status is not exactly recoverable")
    return {
...
def _validate_existing_intents(claim_id: str, root_call_id: str) -> None:
    ...
    if observed_paths != expected_paths:
        raise RuntimeError("V15B existing intent set is partial or unexpected")
    for slot, (condition, seed) in enumerate(
        ...
        if any(observed.get(key) != value for key, value in expected.items()):
            raise RuntimeError(f"V15B existing intent changed: {path.name}")
        if not isinstance(observed.get("created_at_utc"), str):
            raise RuntimeError(f"V15B existing intent lacks creation time: {path.name}")
...
def claim_attempt(claim_id: str, preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if any(REMOTE_STATE.iterdir()):
        raise RuntimeError("registered V15B Volume is not fresh and empty")
    value = {
        "schema_version": 1,
        "protocol": PROTOCOL,
        "scientific_status": "development_only_adaptive_v14_horizon_replication",
        "claim_id": claim_id,
        "registration_sha256": REGISTRATION_SHA256,
        "metric_schema_sha256": METRIC_SCHEMA_SHA256,
        "seeds": list(SEEDS),
    ...
    while time.monotonic() < deadline:
        ...
        if path.is_file():
            ...
            if value.get("claim_id") == claim_id:
                return value
        time.sleep(1)
    raise RuntimeError("durable V15B launch receipt did not arrive")


def validate_history_rows(rows: Sequence[Mapping[str, Any]]) -> dict[int, dict[str, Any]]:
    if [row.get("step") for row in rows] != list(STEPS):
        raise RuntimeError("validation history must contain exact ordered steps 0..5")
...
def verify_log_history(
    ...
    if [row.get("step") for row in reward_rows] != list(POST_BASELINE_STEPS):
        raise RuntimeError("Trainer history lacks exact reward steps 1..5")
    ...
    if [row.get("step") for row in validation_rows] != list(POST_BASELINE_STEPS):
        raise RuntimeError(
            "Trainer validation rows must be exactly steps 1..5; baseline 0 is pre-train"
        )
    ...
    if len(terminal_rows) != 1 or terminal_rows[0].get("step") != 5:
        raise RuntimeError("Trainer history lacks its unique step-5 terminal row")
    return {
...
def _verify_training_outputs(condition: str, seed: int) -> dict[str, Any]:
    ...
    if (
        result.get("completed_updates") != 5
...
def train_run(condition: str, seed: int, claim_id: str) -> dict[str, Any]:
    ...
    if completed.returncode:
        ...
        state_volume.commit()
        raise RuntimeError(f"V15B training failed closed for {label}")
...
def aggregate_results(
    ...
    treatment_terminal = [
        by_step[5]["treatment_by_seed"][str(seed)] for seed in SEEDS
    ]
    signflip_terminal = [
        by_step[5]["signflip_by_seed"][str(seed)] for seed in SEEDS
    ]
    treatment_baseline = [
    ...
    shape_gate["first_above_initial"] = (
        by_step[3]["treatment_mean"] > by_step[0]["treatment_mean"]
    )
    shape_gate["no_downward_steps_3_to_4_to_5"] = (
        by_step[4]["treatment_mean"] >= by_step[3]["treatment_mean"]
        and by_step[5]["treatment_mean"] >= by_step[4]["treatment_mean"]
    )
    shape_gate["passed"] = bool(
        shape_gate["first_above_initial"]
        and shape_gate["no_downward_steps_3_to_4_to_5"]
    )
    ...
    return {
        ...
        "multiplicity_caveat": (
            ...
            "horizon, V11-V14, and seed195 outcomes are not familywise-corrected."
        ),
        "protected_final_payloads_accessed": False,
    }
...
def _write_aggregate_files(aggregate: Mapping[str, Any]) -> tuple[Path, Path, Path]:
    ...
    json_path = evidence / "v15b_celebration_h5_aggregate.json"
    csv_path = evidence / "v15b_celebration_h5_curve.csv"
    png_path = evidence / "v15b_celebration_h5_curve.png"
    ...
    curve_axis.set_title("V14-selected celebration reward: exposed development curve")
...
def _publish_aggregate_to_wandb(
    ...
    try:
        ...
        run.summary["display_shape_gate_passed"] = aggregate["display_shape_gate"]["passed"]
        ...
        logged = run.log_artifact(artifact, aliases=["latest", "v14-selected"])
...
def orchestrate(claim_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if receipt.get("root_call_id") != root_call_id:
            raise RuntimeError("V15B orchestrator lacks durable root authority")
        _read_claim(claim_id)
        state_volume.reload()
        dispatch_dir = REMOTE_STATE / "dispatches"
        ...
        if status_path.is_file():
            ...
            raise RuntimeError(
                "V15B found intents without a complete dispatch status; "
        ...
        if failures:
            raise RuntimeError(
                "V15B workers failed after all eight calls were drained: "
                + " | ".join(failures)
            )
...
def _local_preflight() -> dict[str, Any]:
    ...
    if status or head != pushed:
        raise RuntimeError("V15B launch requires an exact clean pushed main")
    for relative, expected in EXPECTED_FILE_SHA256.items():
    ...
    if modal_executable is None:
        raise RuntimeError("V15B preflight cannot locate the Modal CLI on PATH")
    ...
    if inventory:
        raise RuntimeError("V15B Volume must be fresh and empty")
