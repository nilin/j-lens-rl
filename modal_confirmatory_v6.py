"""Run the conditional celebration-taper V6 confirmation on Modal.

This runner is intentionally unusable until the local V6 protocol has been
...
def _protocol(
    ...
    return _run(
        [sys.executable, "scripts/confirmatory_v6_protocol.py", command, *extra],
        check=check,
    )
...
def _verify_attempt_claim(claim_id: str) -> dict[str, Any]:
    ...
    if not path.is_file():
        raise RuntimeError("V6 volume has no attempt claim")
    claim = json.loads(path.read_text())
    if claim.get("claim_id") != claim_id:
        raise RuntimeError("V6 volume is claimed by another launch")
    return claim
    ...
    while time.monotonic() < deadline:
        ...
        if path.is_file():
            ...
            if decision.get("claim_id") != claim_id:
                raise RuntimeError("V6 launch receipt belongs to another claim")
            ...
            raise RuntimeError("V6 launch receipt marker is malformed")
        time.sleep(2)
    closure = {
        "claim_id": claim_id,
...
def _validate_operational_preflight(value: Any) -> dict[str, Any]:
    if not isinstance(value, dict):
        raise RuntimeError("V6 launch lacks its global-GPU operational preflight")
    if value.get("exclusive_gpu_confirmation") != GPU_EXCLUSIVE_CONFIRMATION:
        raise RuntimeError("V6 launch was not confirmed exclusive of other GPU apps")
...
def claim_attempt(claim_id: str, operational_preflight: dict[str, Any]) -> dict[str, Any]:
    ...
    if stale:
        raise RuntimeError(f"V6 volume already contains attempt data: {stale}")
    ...
    try:
        ...
        raise RuntimeError("V6 volume is already claimed") from error
...
def record_launch_receipt(
    ...
    try:
        ...
        raise RuntimeError("V6 launch receipt already exists") from error
    state_volume.commit()
    return receipt
...
def train_config(condition: str, seed: int) -> dict[str, Any]:
    if condition not in {"jlens", "signflip"} or seed not in SEEDS:
        raise ValueError("training input is outside registered V6")
    state_volume.reload()
    _protocol("verify")
...
def protocol_step(command: str) -> dict[str, Any]:
    if command not in {"verify-semantic", "curve", "unlock"}:
        raise ValueError("unsupported V6 protocol step")
    state_volume.reload()
...
def evaluate_label(label: str, collection_id: str) -> dict[str, Any]:
    if label not in FINAL_LABELS:
        raise ValueError("evaluation label is outside registered V6")
    state_volume.reload()
...
def analyze_final(collection_id: str) -> dict[str, Any]:
    ...
    if output.exists():
        raise FileExistsError(f"refusing to overwrite V6 analysis: {output}")
    try:
...
def _mapped_results(function: Any, *inputs: Iterable[Any]) -> list[Any]:
    ...
    if failures:
        raise RuntimeError(f"{len(failures)} mapped V6 jobs failed: {failures}")
    return results
...
def orchestrate(claim_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if semantic_check.get("returncode") or not semantic_check.get("verified"):
            raise RuntimeError(f"V6 semantic verification failed: {semantic_check}")
        ...
        if unlock.get("returncode"):
            raise RuntimeError(f"V6 unlock failed: {unlock}")
        ...
        if collection.get("returncode") or collection.get("collection_id") != collection_id:
            raise RuntimeError(f"V6 final collection claim failed: {collection}")
        state_volume.reload()
        _record_attempt_status(
            claim_id,
            "sealed_evaluation",
...
def _upload_protocol_state() -> None:
    subprocess.run(
        [sys.executable, "scripts/confirmatory_v6_protocol.py", "verify"],
        cwd=LOCAL_REPO,
        check=True,
    )
    ...
    if actual_counts != expected_counts:
        raise RuntimeError(f"prepared V6 upload inventory changed: {actual_counts}")
...
def _local_operational_preflight() -> dict[str, Any]:
    ...
    if confirmation != GPU_EXCLUSIVE_CONFIRMATION:
        raise RuntimeError(
            "refusing V6 launch without an external no-overlap preflight; ensure "
    ...
    if active_other_apps:
        raise RuntimeError(
            "refusing V6 launch while another Modal app remains active; stop it "
