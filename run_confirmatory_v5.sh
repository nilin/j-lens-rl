...
PROTOCOL=("$PYTHON" scripts/confirmatory_v5_protocol.py)
STATE=.confirmatory/v5
SEEDS=(168 169 170 171 172 173 174 175)
export JLENS_MODAL_IMAGE_SPEC="${JLENS_MODAL_IMAGE_SPEC:-j-lens-rl-confirmatory-v5-emotional-image-v1}"
...
eval_if_missing() {
  ...
  if [[ -e "$output" ]]; then
    ...
    echo "reusing complete, verified V5 evaluation: $output"
    return 0
  fi
  "$EVAL" \
...
final_evaluation() {
  ...
  collection_id="$($PYTHON -c 'import json; print(json.load(open(".confirmatory/v5/final_collection.json"))["collection_id"])')"
  ...
  eval_if_missing \
    ...
    base \
    "$STATE/configs/jlens_seed168.json"
  ...
  if [[ -e "$STATE/evidence/sealed_comparison.json" ]]; then
    echo "refusing to overwrite V5 sealed comparison"
    return 2
  fi
  local analysis_args=(
    "${compare_args[@]}"
...
case "${1:-}" in
  ...
  modal)
    ...
    .venv/bin/modal run --detach modal_confirmatory_v5.py
    ;;
  *)
    usage
    exit 2
